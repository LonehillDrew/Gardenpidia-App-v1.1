...
(function () {
  ...
  // Render plain text into safe paragraphs, preserving line breaks. No markdown parsing.
  ...
  function renderFacts(items) {
    ...
    summary.innerHTML =
      ...
      `<strong>${passes}</strong> within format, ` +
  ...
  async function factFinderRun() {
    ...
    const mode  = $("#ff-mode").value; // "all" or a single category name
    const btn   = $("#ff-run");
    const out   = $("#ff-output");
  ...
  function settingsInit() {
    ...
    $("#set-test").addEventListener("click", async () => {
      ...
      STORE.key = keyInput.value.trim(); STORE.model = sel.value; refreshStatus();
      if (!STORE.key) { res.textContent = "Enter a key first."; res.className = "test-result error-note"; return; }
  ...
  function init() {
    ...
    $("#chat-clear").addEventListener("click", () => { chat.messages = []; chatRender(); });
    chatRender();
    ...
    $("#ff-copy").addEventListener("click", () => {
      ...
      navigator.clipboard.writeText(txt).then(() => flash("Facts copied."));
    });
